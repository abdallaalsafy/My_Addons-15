...
class cls_sells_goods(models.Model):
    _inherit = 'mdl_sells_goods'
    _description = 'Table Of All Sells Goods'
    _sql_constraints = [('CheckSellsGoodsPriceQest', 'check (fld_price_qst > 0)', 'Goods Price_Qest Must Be Above Zero')]


    fld_is_qst = fields.Boolean(related="fld_sells_id.fld_is_qst")
    # Calculated Fields
    fld_price_qst = fields.Float(string='Price Qest', compute='fnc_compute_qest_prices_unit', store=True,
                                 readonly=False)
    fld_hafez = fields.Float(string='Hafez', compute='fnc_compute_qest_prices_unit', store=True, readonly=False)

    fld_clc_qst = fields.Float(string='Calc Qest', compute='fnc_compute_qest_totals', store=True)
    fld_win_qst = fields.Float(string='Winning Qest', compute='fnc_compute_qest_totals', store=True)
    fld_clc_hafez = fields.Float(string='Calc Hafez', compute='fnc_compute_qest_totals', store=True)

    # ======================= Compute Functions =======================
    @api.depends('fld_goods_id', 'fld_unit_id')
    def fnc_compute_qest_prices_unit(self):
    ...
    def fnc_compute_qest_totals(self):
