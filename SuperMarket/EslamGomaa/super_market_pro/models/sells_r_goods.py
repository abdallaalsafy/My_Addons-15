...
class cls_sells_r_goods(models.Model):
    _name = 'mdl_sells_r_goods'
    _description = 'Table Of All Return Sells Goods'
    _sql_constraints = [('CheckReturnSellsGoodsCount', 'check (fld_count>0)', 'Goods Count Must Be Above Zero'),
                        ('CheckReturnSellsGoodsPriceBuy', 'check (fld_price_buy>0)', 'Goods Price_Buy Must Be Above Zero'),
                        ('CheckReturnSellsGoodsPriceSell', 'check (fld_price_sell>0)', 'Goods Price_Sell Must Be Above Zero')]
    ...
    fld_price_buy = fields.Float(string='Price Buy',  compute='fnc_compute_prices_unit', store=True,
                                 readonly=True)
    fld_price_sell = fields.Float(string='Price Sell', readonly=False,  compute='fnc_compute_prices_unit',
                                  store=True)
    ...
    fld_sells_r_id = fields.Many2one('mdl_sells_r', string="Return Sells ID", required=True, ondelete='restrict')

    # This fields for search in buys return goods
    fld_customer_id = fields.Many2one(related="fld_sells_r_id.fld_customer_id")
    fld_date = fields.Date(related="fld_sells_r_id.fld_date")
    ...
    def fnc_compute_totals(self):
        """Calculate totals and winning (negative for returns)"""
        for rcrd in self:
