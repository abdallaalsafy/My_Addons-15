...
class cls_vendors_out(models.Model):
    _name = 'mdl_vendors_out'
    _description = 'Table Of All Vendor Payments'
    _order = "id desc"
    ...
    fld_buys_id = fields.Many2one('mdl_buys', string="Purchase Order", ondelete='cascade')
    ...
    # ======================= Onchange Functions =======================
    @api.onchange('fld_buys_id')
    def fnc_chng_buys_id(self):
        """Set vendor when purchase order changes"""
        buys_id = self.fld_buys_id
        if buys_id:
            self.fld_vendor_id = buys_id.fld_vendor_id

    # ======================= Built-in Functions =======================
    @api.model_create_multi
    def create(self, vals_list):
        """Create vendor payment records with sequence"""
        for vals in vals_list:
            vals['name'] = self.env['ir.sequence'].next_by_code('out_sqnce')
        return super(cls_vendors_out, self).create(vals_list)
    ...
    def unlink(self):
        ...
        return super(cls_vendors_out, self).unlink()
